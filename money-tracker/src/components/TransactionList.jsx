  ...
  return (
    <div className="mt-8 bg-white rounded-2xl shadow-lg p-8">
      <div className="flex gap-4 mb-6 items-center">
        ...
        <label className="text-gray-700 font-medium ml-4">Type:</label>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border rounded-2xl overflow-hidden shadow-sm">
          <thead className="bg-gray-100">
            <tr>
              <th className="border px-4 py-3 text-left font-semibold">Title</th>
              ...
              <th className="border px-4 py-3 text-left font-semibold">Actions</th>
            </tr>
          </thead>
          <tbody>
            {transactions.length === 0 && (
              <tr>
                <td colSpan={6} className="text-center py-12 text-gray-400 text-lg font-medium bg-gray-50">No transactions found. Add your first transaction!</td>
              </tr>
            )}
            {transactions.map((tx, idx) => (
              ...
              >
                <td className="border px-4 py-3 font-medium text-gray-800">{tx.title}</td>
                <td className="border px-4 py-3 text-gray-700">{tx.category}</td>
                <td className="border px-4 py-3 text-blue-700 font-semibold">₹{tx.amount}</td>
                <td className="border px-4 py-3 capitalize text-sm font-semibold {tx.type === 'income' ? 'text-green-600' : 'text-red-600'}">
                  {tx.type}
                </td>
                <td className="border px-4 py-3 text-gray-600">{new Date(tx.date.seconds ? tx.date.seconds * 1000 : tx.date).toLocaleDateString()}</td>
                <td className="border px-4 py-3 flex gap-2 items-center">
                  <button
